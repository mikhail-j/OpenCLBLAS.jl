...
main(void)
{
	...
	printf("clGetPlatformIDs() found %i devices!\n",num);
	printf("clGetPlatformIDs() found %i devices!\n",num);
    err = clGetPlatformIDs(2, &platforms, &num);
	printf("clGetPlatformIDs() found %i devices!\n",num);
    if (err != CL_SUCCESS) {
    ...
    err = clGetDeviceIDs(platform, CL_DEVICE_TYPE_ALL, 1, &device, NULL);
		err = clGetDeviceInfo(device, CL_DEVICE_NAME, sizeof(dev_name), dev_name, NULL);
		printf("%s\n", dev_name);
	//cl_device_id* device = (cl_device_id*)malloc(sizeof(cl_device_id)*num);
   // err = clGetDeviceIDs(platform[1], CL_DEVICE_TYPE_ALL, 1, &device, NULL);
	//	err = clGetDeviceInfo(device, CL_DEVICE_NAME, sizeof(dev_name), dev_name, NULL);
	//	printf("%s\n", dev_name);
    if (err != CL_SUCCESS) {
        printf( "clGetDeviceIDs() failed with %d\n", err );
		if (err == CL_INVALID_PLATFORM) {printf("invalid platform");}
		if (err == CL_INVALID_DEVICE_TYPE) {printf("invalid device");}
		if (err == CL_INVALID_VALUE) {printf("invalid value");}
		if (err == CL_DEVICE_NOT_FOUND) {printf("device not found!");}
        return 1;
    }
	else {
		err = clGetDeviceInfo(device, CL_DEVICE_NAME, sizeof(dev_name), dev_name, NULL);
