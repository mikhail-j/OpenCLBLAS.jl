#=**
* Ported from OpenCLBLAS cHER2 example (example_zher2.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasCher2(order,uplo,N,alpha,X,offx,incX,Y,offy,incy,A,offA,lda,ncq,cq,ne,wle,e)
	return ccall((:clblasCher2, libclblas), cl_int, (clblasOrder,
		clblasUplo,
		Csize_t,
		clblasFloatComplex,
		cl_mem,#X
...
function main()
	...
	A = [hcat(Complex{Float32}(11.0, 00.0), Complex{Float32}(12.0, 02.0), Complex{Float32}(13.0, 05.0), Complex{Float32}(14.0, 12.0), Complex{Float32}(15.0, 06.0));hcat(Complex{Float32}(00.0, 00.0), Complex{Float32}(22.0, 00.0), Complex{Float32}(23.0, 25.0), Complex{Float32}(24.0, 23.0), Complex{Float32}(25.0, 61.0));hcat(Complex{Float32}(00.0, 00.0), Complex{Float32}(00.0, 00.0), Complex{Float32}(33.0, 00.0), Complex{Float32}(34.0, 23.0), Complex{Float32}(35.0, 21.0));hcat(Complex{Float32}(00.0, 00.0), Complex{Float32}(00.0, 00.0), Complex{Float32}(00.0, 00.0), Complex{Float32}(44.0, 00.0), Complex{Float32}(45.0, 23.0));hcat(Complex{Float32}(00.0, 00.0), Complex{Float32}(00.0, 00.0), Complex{Float32}(00.0, 00.0), Complex{Float32}(00.0, 00.0), Complex{Float32}(55.0, 00.0))]
	X = [Complex{Float32}(11.0, 03.0),Complex{Float32}(01.0, 15.0),Complex{Float32}(30.0, 20.0),Complex{Float32}(01.0, 02.0),Complex{Float32}(11.0, 10.0)]
	Y = [Complex{Float32}(11.0, 03.0),Complex{Float32}(03.0, 05.0),Complex{Float32}(09.0, 00.0),Complex{Float32}(01.0, 02.0),Complex{Float32}(11.0, 00.0)]
	...
	err = Array(cl_int, 1)
	bufA = clCreateBuffer(ctx, CL_MEM_READ_WRITE, N * N * sizeof(clblasFloatComplex), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
	...
	event[1] = C_NULL
	statusCheck(clEnqueueWriteBuffer(queue[1], bufA, CL_TRUE, Csize_t(0), N * N * sizeof(clblasFloatComplex), A1, cl_uint(0), C_NULL, event))
...
=====#
	...
	statusCheck(clblasCher2(order, uplo, N, alpha, bufX, 0, incX, bufY, 0, incY, bufA, 0, N, 1, queue, 0, C_NULL, event))
	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory


	A2=vec(Array(clblasFloatComplex,N*N))
	event[1] = C_NULL
	statusCheck(clEnqueueReadBuffer(queue[1], bufA, CL_TRUE, Csize_t(0), N*N*sizeof(clblasFloatComplex), A2, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
