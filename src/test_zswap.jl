#=**
* Ported from OpenCLBLAS zSWAP example (example_sswap.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasZswap(N,X,offx,incX,Y,offy,incy,ncq,cq,ne,wle,e)
	return ccall((:clblasZswap, libclblas), cl_int, (Csize_t,
		cl_mem,#X
		Csize_t,
		cl_int,
		cl_mem,#Y
		...
		Ref{cl_event},
		Ref{cl_event}),
		N,X,offx,incX,Y,offy,incy,ncq,cq,ne,wle,e)
end
...
function main()
	...
	X = [clblasDoubleComplex(11),clblasDoubleComplex(21),clblasDoubleComplex(31),clblasDoubleComplex(41),clblasDoubleComplex(51),clblasDoubleComplex(61),clblasDoubleComplex(71)]
	Y = [clblasDoubleComplex(45),clblasDoubleComplex(23),clblasDoubleComplex(39),clblasDoubleComplex(45),clblasDoubleComplex(50),clblasDoubleComplex(55),clblasDoubleComplex(65)]
...
=====#
	...
	statusCheck(clblasZswap(N, bufX, 0, incX, bufY, 0, incY, 1, queue, 0, C_NULL, event))
	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	...
	event[1] = C_NULL
	statusCheck(clEnqueueReadBuffer(queue[1], bufX, CL_TRUE, Csize_t(0), N * sizeof(clblasDoubleComplex), X, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	event[1] = C_NULL
	statusCheck(clEnqueueReadBuffer(queue[1], bufY, CL_TRUE, Csize_t(0), N * sizeof(clblasDoubleComplex), Y, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
