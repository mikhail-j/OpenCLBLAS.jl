#=**
* Ported from OpenCLBLAS cTRMV example (example_strmv.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasCtrmv(o,ul,t,d,N,A,offA,lda,X,offX,incX,sb,ncq,cq,ne,wle,e)
	return ccall((:clblasCtrmv, libclblas), cl_int, (clblasOrder,
		clblasUplo,
		clblasTranspose,
		clblasDiag,
		Csize_t,#N
...
function main()
	...
	statusCheck(err[1])
	################################	create arrays
	A = convert(Array{clblasFloatComplex,2}, [hcat(11, 12, 13, 14, 15);hcat(0, 22, 23, 24, 25);hcat(0, 0, 33, 34, 35);hcat(0, 0, 0, 44, 45);hcat(0, 0, 0, 0, 55)]);
	...
	statusCheck(err[1])
	err = Array(cl_int, 1)
	bufX = clCreateBuffer(ctx, CL_MEM_WRITE_ONLY, N * sizeof(clblasFloatComplex), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
	sbuf = clCreateBuffer(ctx, CL_MEM_READ_ONLY, N * sizeof(clblasFloatComplex), C_NULL, err)
	statusCheck(err[1])
...
=#
	event[1] = C_NULL
	statusCheck(clblasCtrmv(order,uplo,trans,diag,N,bufA,Csize_t(0), lda, bufX, Csize_t(0), incX, sbuf, cl_int(1), queue, cl_uint(0), C_NULL, event))
	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory


	X1 = zeros(clblasFloatComplex,Int(N))
	event[1] = C_NULL
	statusCheck(clEnqueueReadBuffer(queue[1], bufX, CL_TRUE, Csize_t(0), N * sizeof(clblasFloatComplex), X1, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
