#=**
* Ported from OpenCLBLAS zdROT example (example_srot.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasZdrot(N,X,offX,incX,Y,offY,incY,C,S,ncq,cq,ne,wle,e)
	return ccall((:clblasZdrot, libclblas), clblasStatus, (Csize_t,
		cl_mem,#X
		Csize_t,
		cl_int,
		cl_mem,#Y
...
function main()
	...
	statusCheck(err[1])
	################################	create arrays
	X = convert(Array{clblasDoubleComplex, 1}, [1, 2, 3, 4, 5]);
	Y = convert(Array{clblasDoubleComplex, 1}, [6, 7, 8, 9, 9]);
	C = cl_double(2);
	S = cl_double(3);
	incX = cl_int(1);
	incY = cl_int(1);
...
=====#
	...
	statusCheck(clblasZdrot(N, bufX, 0, incX, bufY, 0, incY, C, S, 1, queue, 0, C_NULL, event))
