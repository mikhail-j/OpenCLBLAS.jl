#=**
* Ported from OpenCLBLAS cTPMV example (example_stpmv.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasCtpmv(o,ul,tA,diag,N,A,offA,X,offX,incX,sb,ncq,cq,ne,wle,e)
	return ccall((:clblasCtpmv, libclblas), cl_int, (clblasOrder,
		clblasUplo,
		clblasTranspose,
		clblasDiag,
		Csize_t,#N
...
function main()
	...
	statusCheck(err[1])
	################################	create arrays
	A =	convert(Array{clblasFloatComplex,1}, [11, 12, 13, 14, 15, 22, 23, 24, 25, 33, 34, 35, 44, 45, 55])
...
#n=-.5+.5/sqrt(1+4*2*length(A))
	...
	event[1] = C_NULL
	statusCheck(clEnqueueWriteBuffer(queue[1], bufA, CL_TRUE, Csize_t(0), length(A) * sizeof(clblasFloatComplex), A, cl_uint(0), C_NULL, event))
...
=====#
	...
	statusCheck(clblasCtpmv(order, uploA, transA, diagA,  N, bufA, 0, bufX, 0, incX, sbuf, 1, queue, 0, C_NULL, event))
