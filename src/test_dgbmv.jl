#=**
* Ported from OpenCLBLAS dGBMV example (example_sgemm.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasDgbmv(o,t,M,N,kl,ku,alpha,A,oA,lda,X,oX,iX,beta,Y,oY,iY,ncq,cq,ne,wle,e)
	return ccall((:clblasDgbmv, libclblas), clblasStatus, (clblasOrder,
		clblasTranspose,
		Csize_t,
		Csize_t,
		Csize_t,
...
function main()
	...
	statusCheck(err[1])
	################################	create arrays
	local A = convert(Array{cl_double,2}, [hcat(00, 12, 13, 14);hcat(21, 22, 23, 24);hcat(31, 32, 33, 34);hcat(41, 42, 43, 00);hcat(51, 62, 00, 00)]);
	local X = convert(Array{cl_double,1}, [11,21,31,41,51])
	local Y = convert(Array{cl_double,1}, [11,21,31,41,51])
	local A1 = vec(A')
	...
	local alpha = convert(cl_double, 10)
	local beta = convert(cl_double, 20)
	...
	err = Array(cl_int, 1)
	bufA = clCreateBuffer(ctx, CL_MEM_READ_ONLY, M * lda * sizeof(cl_double), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
	bufX = clCreateBuffer(ctx, CL_MEM_READ_ONLY, N * sizeof(cl_double), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
	bufY = clCreateBuffer(ctx, CL_MEM_READ_WRITE, M * sizeof(cl_double), C_NULL, err)
	statusCheck(err[1])
	statusCheck(clFlush(queue[1]))
	...
	event[1] = C_NULL
	statusCheck(clEnqueueWriteBuffer(queue[1], bufA, CL_TRUE, Csize_t(0), M * lda * sizeof(cl_double), A1, cl_uint(0), C_NULL, event))
	...
	event[1] = C_NULL
	statusCheck(clEnqueueWriteBuffer(queue[1], bufY, CL_TRUE, Csize_t(0), M * sizeof(cl_double), Y, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
...
=====#
	event[1] = C_NULL
	statusCheck(clblasDgbmv(order, trans, M, N, KL, KU, alpha, bufA, 0, lda, bufX, 0, incX, beta, bufY, 0, incY, 1, queue, 0, C_NULL, event))
	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	...
	event[1] = C_NULL
	statusCheck(clEnqueueReadBuffer(queue[1], bufY, CL_TRUE, Csize_t(0), M * sizeof(cl_double), Y, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
...
if (!isempty(libclblas) && !isempty(libopencl))
	#=
	A =	convert(Array{cl_double,2}, [hcat(00, 12, 13, 14);hcat(21, 22, 23, 24);hcat(31, 32, 33, 34);hcat(41, 42, 43, 00);hcat(51, 62, 00, 00)]);
	X = convert(Array{cl_double,1}, [11,21,31,41,51])
	Y = convert(Array{cl_double,1}, [11,21,31,41,51])
