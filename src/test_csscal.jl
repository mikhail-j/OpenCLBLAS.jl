#=**
* Ported from OpenCLBLAS cSSCAL example (example_csscal.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasCsscal(N,alpha,X,offx,incX,ncq,cq,ne,wle,e)
	return ccall((:clblasCsscal, libclblas), cl_int, (Csize_t,
		cl_float,
		cl_mem,#X
		Csize_t,
		cl_int,
...
function main()
	...
	X = [clblasFloatComplex(1),clblasFloatComplex(2),clblasFloatComplex(3),clblasFloatComplex(4),clblasFloatComplex(5)]
...
=====#
	...
	statusCheck(clblasCsscal(N, alpha, bufX, 0, incX, 1, queue, 0, C_NULL, event))
	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
