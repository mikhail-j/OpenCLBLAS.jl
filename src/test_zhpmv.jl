#=**
* Ported from OpenCLBLAS zHPMV example (example_chpmv.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasZhpmv(order,uplo,N,alpha,A,offA,X,offX,incX,beta,Y,offY,incY,ncq,cq,ne,wle,e)
	return ccall((:clblasZhpmv, libclblas), cl_int, (clblasOrder,
		clblasUplo,
		Csize_t,#N
...
clblasDoubleComplex( 1.0, 00.0), clblasDoubleComplex( 2.0, 02.0), clblasDoubleComplex( 4.0,  4.0), clblasDoubleComplex( 7.0,  7.0), clblasDoubleComplex(11.0, 11.0),
								clblasDoubleComplex( 3.0, 03.0), clblasDoubleComplex( 5.0,  5.0), clblasDoubleComplex( 8.0,  8.0), clblasDoubleComplex(12.0, 12.0),
																clblasDoubleComplex( 6.0,  6.0), clblasDoubleComplex( 9.0,  9.0), clblasDoubleComplex(13.0, 13.0),
																								clblasDoubleComplex(10.0, 10.0), clblasDoubleComplex(14.0, 14.0),
																																clblasDoubleComplex(15.0, 15.0)]);
	...
	beta[1] = clblasDoubleComplex(2,2);
	#Now initialize OpenCLBLAS and buffers
	statusCheck(clblasSetup())
	statusCheck(clFlush(queue[1]))
	err = Array(cl_int, 1)
	bufAP = clCreateBuffer(ctx, CL_MEM_READ_ONLY, length(A) * sizeof(clblasDoubleComplex), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
...
=====#
	...
	statusCheck(clblasZhpmv(order, uplo, N, alpha, bufAP, 0,
	...
	Y2=Array(clblasDoubleComplex, N)
	event[1] = C_NULL
