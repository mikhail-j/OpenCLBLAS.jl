#=**
* Ported from OpenCLBLAS sSYMV example (example_ssymv.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasSsymv(o,ul,N,alpha,A,offA,lda,X,offX,incX,beta,Y,offY,incY,ncq,cq,ne,wle,e)
	return ccall((:clblasSsymv, libclblas), cl_int, (clblasOrder,
		clblasUplo,
		Csize_t,#N
		cl_float,#alpha
		cl_mem,#A
		...
		Ptr{cl_event},
		Ptr{cl_event}),
		o,ul,N,alpha,A,offA,lda,X,offX,incX,beta,Y,offY,incY,ncq,cq,ne,wle,e)
end
...
=====#
	...
	statusCheck(clblasSsymv(order, uplo, N, alpha, bufA, 0, lda, bufX, 0, incX, beta, bufY, 0, incY, 1, queue, 0, C_NULL, event))
