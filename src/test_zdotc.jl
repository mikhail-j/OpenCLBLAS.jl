#=**
* Ported from OpenCLBLAS zDOTc example (example_sdot.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasZdotc(N,DP,offDP,X,offX,incX,Y,offY,incY,sb,ncq,cq,ne,wle,e)
	return ccall((:clblasZdotc, libclblas), cl_int, (Csize_t,#N
		cl_mem,#dot product
		Csize_t,
		cl_mem,#X
		Csize_t,
...
function main()
	...
	Y = convert(Array{clblasDoubleComplex, 1}, [1,5,6,4,9,10,4]);
	N = Csize_t(length(X));
	incX = cl_int(1);
	incY = cl_int(1);
	local dotP = zeros(clblasDoubleComplex,1);
	...
	statusCheck(err[1])
	err = Array(cl_int, 1)
	local bufY = clCreateBuffer(ctx, CL_MEM_READ_ONLY, N * sizeof(clblasDoubleComplex), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
	local sbuf = clCreateBuffer(ctx, CL_MEM_READ_WRITE, N * sizeof(clblasDoubleComplex), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
	local bufDot = clCreateBuffer(ctx, CL_MEM_WRITE_ONLY, sizeof(clblasDoubleComplex), C_NULL, err)
	statusCheck(err[1])
	statusCheck(clFlush(queue[1]))
...
=====#
	event[1] = C_NULL
	statusCheck(clblasZdotc(N, bufDot, Csize_t(0), bufX, Csize_t(0), incX, bufY, Csize_t(0), incY, sbuf,1, queue, 0, C_NULL, event))
	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	...
	event[1] = C_NULL
	statusCheck(clEnqueueReadBuffer(queue[1], bufDot, CL_TRUE, Csize_t(0), sizeof(clblasDoubleComplex), dotP, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
