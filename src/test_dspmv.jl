#=**
* Ported from OpenCLBLAS dSPMV example (example_sspmv.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasDspmv(o,ul,N,alpha,A,offA,X,offX,incX,beta,Y,offY,incY,ncq,cq,ne,wle,e)
	return ccall((:clblasDspmv, libclblas), cl_int, (clblasOrder,
		clblasUplo,
		Csize_t,#N
		cl_double,#alpha
		cl_mem,#A
		...
		Ptr{cl_event},
		Ptr{cl_event}),
		o,ul,N,alpha,A,offA,X,offX,incX,beta,Y,offY,incY,ncq,cq,ne,wle,e)
end
...
function main()
	...
	statusCheck(err[1])
	################################	create arrays
	A =	convert(Array{cl_double,1}, vec(hcat(hcat(11, 12, 13, 14, 15),hcat(22, 23, 24, 25),hcat(33, 34, 35),hcat(44, 45),hcat(55))));
...
#n=-.5+.5/sqrt(1+4*2*length(A))
	...
	err = Array(cl_int, 1)
	bufA = clCreateBuffer(ctx, CL_MEM_READ_ONLY, length(A) * sizeof(cl_double), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
	...
	event[1] = C_NULL
	statusCheck(clEnqueueWriteBuffer(queue[1], bufA, CL_TRUE, Csize_t(0), length(A) * sizeof(cl_double), A, cl_uint(0), C_NULL, event))
...
=====#
	...
	statusCheck(clblasDspmv(order, uplo, N, alpha, bufA, 0, bufX, 0, incX, beta, bufY, 0, incY, 1, queue, 0, C_NULL, event))

	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory

	Y2=Array(cl_double, N);
	event[1] = C_NULL
