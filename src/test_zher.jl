#=**
* Ported from OpenCLBLAS zHER example (example_cher.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasZher(order,uplo,N,alpha,X,offX,incX,A,offA,lda,ncq,cq,ne,wle,e)
	return ccall((:clblasZher, libclblas), cl_int, (clblasOrder,
		clblasUplo,
		Csize_t,#N
		cl_double,#alpha
		cl_mem,#X
...
hcat(clblasDoubleComplex(11.0, 00.0), clblasDoubleComplex(12.0, 02.0), clblasDoubleComplex(13.0, 05.0), clblasDoubleComplex(14.0, 12.0), clblasDoubleComplex(15.0, 06.0));
hcat(clblasDoubleComplex(00.0, 00.0), clblasDoubleComplex(22.0, 00.0), clblasDoubleComplex(23.0, 25.0), clblasDoubleComplex(24.0, 23.0), clblasDoubleComplex(25.0, 61.0));
hcat(clblasDoubleComplex(00.0, 00.0), clblasDoubleComplex(00.0, 00.0), clblasDoubleComplex(33.0, 00.0), clblasDoubleComplex(34.0, 23.0), clblasDoubleComplex(35.0, 21.0));
hcat(clblasDoubleComplex(00.0, 00.0), clblasDoubleComplex(00.0, 00.0), clblasDoubleComplex(00.0, 00.0), clblasDoubleComplex(44.0, 00.0), clblasDoubleComplex(45.0, 23.0));
hcat(clblasDoubleComplex(00.0, 00.0), clblasDoubleComplex(00.0, 00.0), clblasDoubleComplex(00.0, 00.0), clblasDoubleComplex(00.0, 00.0), clblasDoubleComplex(55.0, 00.0))]);
	X = convert(Array{clblasDoubleComplex,1}, [
	clblasDoubleComplex(11.0, 23.0);
	clblasDoubleComplex(21.0, 65.0);
	clblasDoubleComplex(31.0, 20.0);
	clblasDoubleComplex(41.0, 02.0);
	clblasDoubleComplex(51.0, 10.0)]);
	...
	err = Array(cl_int, 1)
	bufA = clCreateBuffer(ctx, CL_MEM_READ_WRITE, N * lda * sizeof(clblasDoubleComplex), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
...
=====#
	...
	statusCheck(clblasZher(order, uplo, N, alpha, bufX, 0, incX, bufA, 0, lda, 1, queue, 0, C_NULL, event))

	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory

	A2=Array(clblasDoubleComplex, N * N)
	event[1] = C_NULL
	statusCheck(clEnqueueReadBuffer(queue[1], bufA, CL_TRUE, Csize_t(0), N * lda * sizeof(clblasDoubleComplex), A2, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
