#=**
* Ported from OpenCLBLAS sSCAL example (example_sscal.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasSscal(N,alpha,X,offx,incX,ncq,cq,ne,wle,e)
	return ccall((:clblasSscal, libclblas), cl_int, (Csize_t,
		cl_float,
		cl_mem,#X
		Csize_t,
		cl_int,
...
function main()
	...
	X = [cl_float(11),cl_float(21),cl_float(31),cl_float(41),cl_float(51)]
...
=====#
	...
	statusCheck(clblasSscal(N, alpha, bufX, 0, incX, 1, queue, 0, C_NULL, event))
	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory

	X1=vec(Array(cl_float,N))
	event[1] = C_NULL
