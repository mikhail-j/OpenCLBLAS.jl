#=**
* Ported from OpenCLBLAS dTPMV example (example_stpmv.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasDtpmv(o,ul,tA,diag,N,A,offA,X,offX,incX,sb,ncq,cq,ne,wle,e)
	return ccall((:clblasDtpmv, libclblas), cl_int, (clblasOrder,
		clblasUplo,
		clblasTranspose,
		clblasDiag,
		Csize_t,#N
		...
		Ptr{cl_event},
		Ptr{cl_event}),
		o,ul,tA,diag,N,A,offA,X,offX,incX,sb,ncq,cq,ne,wle,e)
end
...
function main()
	...
	statusCheck(err[1])
	################################	create arrays
	A =	convert(Array{cl_double,1}, [11, 12, 13, 14, 15, 22, 23, 24, 25, 33, 34, 35, 44, 45, 55])
...
=====#
	...
	statusCheck(clblasDtpmv(order, uploA, transA, diagA,  N, bufA, 0, bufX, 0, incX, sbuf, 1, queue, 0, C_NULL, event))
