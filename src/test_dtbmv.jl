#=**
* Ported from OpenCLBLAS dTBMV example (example_stbmv.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasDtbmv(o,ul,t,diag,N,K,A,offA,lda,X,offX,incX,sb,ncq,cq,ne,wle,e)
	return ccall((:clblasDtbmv, libclblas), cl_int, (clblasOrder,
		clblasUplo,
		clblasTranspose,
		clblasDiag,
		Csize_t,#N
		...
		Ptr{cl_event},
		Ptr{cl_event}),
		o,ul,t,diag,N,K,A,offA,lda,X,offX,incX,sb,ncq,cq,ne,wle,e)
end
...
function main()
	...
	statusCheck(err[1])
	################################	create arrays
	A =	convert(Array{cl_double,2}, [hcat(11, 12, 13);hcat(22, 23, 24);hcat(33, 34, 35);hcat(44, 45, 00);hcat(55, 00, 00)])
	...
	event[1] = C_NULL
	statusCheck(clEnqueueWriteBuffer(queue[1], bufA, CL_TRUE, Csize_t(0), N * lda * sizeof(cl_double), transposeNC(A), cl_uint(0), C_NULL, event))
...
=====#
	...
	statusCheck(clblasDtbmv(order, uplo, trans, diag, N, K, bufA, 0, lda, bufX, 0, incX, sbuf, 1, queue, 0, C_NULL, event))

	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory

	X2=Array(cl_double,length(X))
	event[1] = C_NULL
