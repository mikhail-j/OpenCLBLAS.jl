#=**
* Ported from OpenCLBLAS dTRSV example (example_strsv.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasDtrsv(o, ul, tA, diag, N, A, offA, lda, X, offX, incX, ncq, cq, ne, wle, e)
	return ccall((:clblasDtrsv, libclblas), cl_int, (clblasOrder,
		clblasUplo,
		clblasTranspose,
		clblasDiag,
		Csize_t,
...
function main()
	...
	statusCheck(err[1])
	################################	create arrays
	A =	convert(Array{cl_double,2}, [hcat(11,  0,  0,  0);hcat(12, 22,  0,  0);hcat(13, 23, 33,  0);hcat(14, 24, 34, 44)])
	X = convert(Array{cl_double,1}, [11,21,31,41])
	##A =	convert(Array{cl_double2,2}, convert(Array{cl_double,2}, [[11, 12, 13, 14, 15]';[21, 22, 23, 24, 25]';[31, 32, 33, 34, 35]';[41, 42, 43, 44, 45]']))
	##B = convert(Array{cl_double2,2}, convert(Array{cl_double,2}, [[11, 12, 13]';[21, 22, 23]';[31, 32, 33]';[41, 42, 43]';[51, 52, 53]']))
	##C = convert(Array{cl_double2,2}, convert(Array{cl_double,2}, [[11, 12, 13]';[21, 22, 23]';[31, 32, 33]';[41, 42, 43]']))
...
=====#
	...
	statusCheck(clblasDtrsv(order, uploA, transA, diagA, N, bufA, 0, lda, bufX, 0, incX, 1, queue, 0, C_NULL, event))

	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory

	X2=Array(cl_double,N)
	event[1] = C_NULL
	statusCheck(clEnqueueReadBuffer(queue[1], bufX, CL_TRUE, Csize_t(0), N * sizeof(cl_double), X2, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
