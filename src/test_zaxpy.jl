#=**
* Ported from OpenCLBLAS zAXPY example (example_saxpy.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasZaxpy(N,alpha,X,offx,incX,Y,offy,incy,ncq,cq,ne,wle,e)
	return ccall((:clblasZaxpy, libclblas), cl_int, (Csize_t,
		clblasDoubleComplex,
		cl_mem,
		Csize_t,
		cl_int,
...
function main()
	...
	statusCheck(err[1])
	################################	create arrays
	X=vec(convert(Array{clblasDoubleComplex, 1},[11,21,31,41,51,61,71]))
	Y=vec(convert(Array{clblasDoubleComplex, 1},[15,11,1,2,1,8,1]))
	N=Csize_t(length(X))
	
	alpha = convert(clblasDoubleComplex, 10)
	...
	err = Array(cl_int, 1)
	bufX = clCreateBuffer(ctx, CL_MEM_READ_ONLY, lenX * sizeof(clblasDoubleComplex), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
	bufY = clCreateBuffer(ctx, CL_MEM_READ_WRITE, lenY * sizeof(clblasDoubleComplex), C_NULL, err)
	statusCheck(err[1])
	statusCheck(clFlush(queue[1]))
	...
	event[1] = C_NULL
	statusCheck(clEnqueueWriteBuffer(queue[1], bufX, CL_TRUE, Csize_t(0), lenX * sizeof(clblasDoubleComplex), X, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	event[1] = C_NULL
	statusCheck(clEnqueueWriteBuffer(queue[1], bufY, CL_TRUE, Csize_t(0), lenY * sizeof(clblasDoubleComplex), Y, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
...
=====#
	...
	statusCheck(clblasZaxpy( N, alpha, bufX, 0, incX, bufY, 0, incY, 1, queue, 0, C_NULL, event))
	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory


	Y2=vec(Array(clblasDoubleComplex,N))
	event[1] = C_NULL
