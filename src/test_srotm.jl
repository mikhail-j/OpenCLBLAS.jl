#=**
* Ported from OpenCLBLAS sROTM example (example_srotm.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasSrotm(N,X,offX,incX,Y,offY,incY,param,offparam,ncq,cq,ne,wle,e)
	return ccall((:clblasSrotm, libclblas), clblasStatus, (Csize_t,
		cl_mem,#X
		Csize_t,
		cl_int,
		cl_mem,#Y
		...
		Ptr{cl_event},
		Ptr{cl_event}),
		N,X,offX,incX,Y,offY,incY,param,offparam,ncq,cq,ne,wle,e)
end
...
function main()
	...
	Y = convert(Array{cl_float,1}, [15,11,1,2,1,8,1]);
	incX = cl_int(1);
	incY = cl_int(1);
	N = Csize_t(length(X));
...
=====#
	...
	statusCheck(clblasSrotm(N, bufX, 0, incX, bufY, 0, incY, bufP, 0, 1, queue, 0, C_NULL, event))
