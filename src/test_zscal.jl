#=**
* Ported from OpenCLBLAS zSCAL example (example_sscal.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasZscal(N,alpha,X,offx,incX,ncq,cq,ne,wle,e)
	return ccall((:clblasZscal, libclblas), cl_int, (Csize_t,
		clblasDoubleComplex,
		cl_mem,#X
		Csize_t,
		cl_int,
...
function main()
	...
	X = [clblasDoubleComplex(11),clblasDoubleComplex(21),clblasDoubleComplex(31),clblasDoubleComplex(41),clblasDoubleComplex(51)]
...
=====#
	...
	statusCheck(clblasZscal(N, alpha, bufX, 0, incX, 1, queue, 0, C_NULL, event))
	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
