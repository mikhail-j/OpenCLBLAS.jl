#=**
* Ported from OpenCLBLAS sROT example (example_srot.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasSrot(N,X,offX,incX,Y,offY,incY,C,S,ncq,cq,ne,wle,e)
	return ccall((:clblasSrot, libclblas), clblasStatus, (Csize_t,
		cl_mem,#X
		Csize_t,
		cl_int,
		cl_mem,#Y
...
function main()
	...
	statusCheck(err[1])
	################################	create arrays
	X = convert(Array{cl_float, 1}, [1, 2, 3, 4, 5]);
	Y = convert(Array{cl_float, 1}, [6, 7, 8, 9, 9]);
	C = cl_float(2);
	S = cl_float(3);
	incX = cl_int(1);
	incY = cl_int(1);
...
=====#
	...
	statusCheck(clblasSrot(N, bufX, 0, incX, bufY, 0, incY, C, S, 1, queue, 0, C_NULL, event))
