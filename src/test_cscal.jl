#=**
* Ported from OpenCLBLAS cSCAL example (example_sscal.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasCscal(N,alpha,X,offx,incX,ncq,cq,ne,wle,e)
	return ccall((:clblasCscal, libclblas), cl_int, (Csize_t,
		clblasFloatComplex,
		cl_mem,#X
		Csize_t,
		cl_int,
...
function main()
	...
	X = [clblasFloatComplex(11),clblasFloatComplex(21),clblasFloatComplex(31),clblasFloatComplex(41),clblasFloatComplex(51)]
...
=====#
	...
	statusCheck(clblasCscal(N, alpha, bufX, 0, incX, 1, queue, 0, C_NULL, event))
	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory

	X1=vec(Array(clblasFloatComplex,N))
	event[1] = C_NULL
