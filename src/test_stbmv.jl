#=**
* Ported from OpenCLBLAS sTBMV example (example_stbmv.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasStbmv(o,ul,t,diag,N,K,A,offA,lda,X,offX,incX,sb,ncq,cq,ne,wle,e)
	return ccall((:clblasStbmv, libclblas), cl_int, (clblasOrder,
		clblasUplo,
		clblasTranspose,
		clblasDiag,
		Csize_t,#N
...
function main()
	...
	statusCheck(err[1])
	err = Array(cl_int, 1)
	bufX = clCreateBuffer(ctx, CL_MEM_WRITE_ONLY, N * sizeof(cl_float), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
	sbuf = clCreateBuffer(ctx, CL_MEM_READ_ONLY, N * sizeof(cl_float), C_NULL, err)
	statusCheck(err[1])
	statusCheck(clFlush(queue[1]))
	...
	event[1] = C_NULL
	statusCheck(clEnqueueWriteBuffer(queue[1], bufA, CL_TRUE, Csize_t(0), N * lda * sizeof(cl_float), transposeNC(A), cl_uint(0), C_NULL, event))
...
=====#
	...
	statusCheck(clblasStbmv(order, uplo, trans, diag, N, K, bufA, 0, lda, bufX, 0, incX, sbuf, 1, queue, 0, C_NULL, event))

	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory

	X2=Array(cl_float,length(X))
	event[1] = C_NULL
