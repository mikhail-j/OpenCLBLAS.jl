#=**
* Ported from OpenCLBLAS sTRMV example (example_strmv.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasStrmv(o,ul,t,d,N,A,offA,lda,X,offX,incX,sb,ncq,cq,ne,wle,e)
	return ccall((:clblasStrmv, libclblas), cl_int, (clblasOrder,
		clblasUplo,
		clblasTranspose,
		clblasDiag,
		Csize_t,#N
...
function main()
	...
	statusCheck(err[1])
	################################	create arrays
	A = convert(Array{cl_float,2}, [hcat(11, 12, 13, 14, 15);hcat(0, 22, 23, 24, 25);hcat(0, 0, 33, 34, 35);hcat(0, 0, 0, 44, 45);hcat(0, 0, 0, 0, 55)]);
...
=#
	event[1] = C_NULL
	statusCheck(clblasStrmv(order,uplo,trans,diag,N,bufA,Csize_t(0), lda, bufX, Csize_t(0), incX, sbuf, cl_int(1), queue, cl_uint(0), C_NULL, event))
	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory


	X1 = zeros(cl_float,Int(N))
	event[1] = C_NULL
	statusCheck(clEnqueueReadBuffer(queue[1], bufX, CL_TRUE, Csize_t(0), N * sizeof(cl_float), X1, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
