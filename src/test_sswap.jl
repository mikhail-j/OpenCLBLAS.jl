#=**
* Ported from OpenCLBLAS sSWAP example (example_sswap.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasSswap(N,X,offx,incX,Y,offy,incy,ncq,cq,ne,wle,e)
	return ccall((:clblasSswap, libclblas), cl_int, (Csize_t,
		cl_mem,#X
		Csize_t,
		cl_int,
		cl_mem,#Y
...
function main()
	...
	X = [cl_float(11),cl_float(21),cl_float(31),cl_float(41),cl_float(51),cl_float(61),cl_float(71)]
	Y = [cl_float(45),cl_float(23),cl_float(39),cl_float(45),cl_float(50),cl_float(55),cl_float(65)]
...
=====#
	...
	statusCheck(clblasSswap(N, bufX, 0, incX, bufY, 0, incY, 1, queue, 0, C_NULL, event))
	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
