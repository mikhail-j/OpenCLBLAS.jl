#=**
* Ported from OpenCLBLAS cHER example (example_cher.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasCher(order,uplo,N,alpha,X,offX,incX,A,offA,lda,ncq,cq,ne,wle,e)
	return ccall((:clblasCher, libclblas), cl_int, (clblasOrder,
		clblasUplo,
		Csize_t,#N
		cl_float,#alpha
		cl_mem,#X
		...
		Ptr{cl_event},
		Ptr{cl_event}),
		order,uplo,N,alpha,X,offX,incX,A,offA,lda,ncq,cq,ne,wle,e)
end
...
hcat(clblasFloatComplex(11.0, 00.0), clblasFloatComplex(12.0, 02.0), clblasFloatComplex(13.0, 05.0), clblasFloatComplex(14.0, 12.0), clblasFloatComplex(15.0, 06.0));
hcat(clblasFloatComplex(00.0, 00.0), clblasFloatComplex(22.0, 00.0), clblasFloatComplex(23.0, 25.0), clblasFloatComplex(24.0, 23.0), clblasFloatComplex(25.0, 61.0));
hcat(clblasFloatComplex(00.0, 00.0), clblasFloatComplex(00.0, 00.0), clblasFloatComplex(33.0, 00.0), clblasFloatComplex(34.0, 23.0), clblasFloatComplex(35.0, 21.0));
hcat(clblasFloatComplex(00.0, 00.0), clblasFloatComplex(00.0, 00.0), clblasFloatComplex(00.0, 00.0), clblasFloatComplex(44.0, 00.0), clblasFloatComplex(45.0, 23.0));
hcat(clblasFloatComplex(00.0, 00.0), clblasFloatComplex(00.0, 00.0), clblasFloatComplex(00.0, 00.0), clblasFloatComplex(00.0, 00.0), clblasFloatComplex(55.0, 00.0))]);
	X = convert(Array{clblasFloatComplex,1}, [
	clblasFloatComplex(11.0, 23.0);
	clblasFloatComplex(21.0, 65.0);
	clblasFloatComplex(31.0, 20.0);
	clblasFloatComplex(41.0, 02.0);
	clblasFloatComplex(51.0, 10.0)]);
	...
	err = Array(cl_int, 1)
	bufA = clCreateBuffer(ctx, CL_MEM_READ_WRITE, N * lda * sizeof(clblasFloatComplex), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
	...
	event[1] = C_NULL
	statusCheck(clEnqueueWriteBuffer(queue[1], bufA, CL_TRUE, Csize_t(0), N * lda * sizeof(clblasFloatComplex), A.', cl_uint(0), C_NULL, event))
...
=====#
	...
	statusCheck(clblasCher(order, uplo, N, alpha, bufX, 0, incX, bufA, 0, lda, 1, queue, 0, C_NULL, event))

	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory

	A2=Array(clblasFloatComplex, N * N)
	event[1] = C_NULL
	statusCheck(clEnqueueReadBuffer(queue[1], bufA, CL_TRUE, Csize_t(0), N * lda * sizeof(clblasFloatComplex), A2, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
