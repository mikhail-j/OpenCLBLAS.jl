#=**
* Ported from OpenCLBLAS sTPSV example (example_stpsv.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasStpsv(o,ul,tA,diag,N,A,offA,X,offX,incX,ncq,cq,ne,wle,e)
	return ccall((:clblasStpsv, libclblas), cl_int, (clblasOrder,
		clblasUplo,
		clblasTranspose,
		clblasDiag,
		Csize_t,#N
...
function main()
	...
	X = convert(Array{cl_float,1}, [11.0,153.0,657.0,1753.0,3671.0])
#n*(n+1)=length(A)
...
=====#
	...
	statusCheck(clblasStpsv(order, uploA, transA, diagA, N, bufA, 0, bufX, 0, incX, 1, queue, 0, C_NULL, event))
