#=**
* Ported from OpenCLBLAS zGERC example (example_sger.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasZgerc(o,M,N,alpha,X,offX,incX,Y,offY,A,offA,lda,incY,ncq,cq,ne,wle,e)
	return ccall((:clblasZgerc, libclblas), cl_int, (clblasOrder,
		Csize_t,#M
		Csize_t,#N
		clblasDoubleComplex,#alpha
		cl_mem,#X
		Csize_t,
		cl_int,
		cl_mem,#Y
...
function main()
	...
	statusCheck(err[1])
	################################	create arrays
	A =	convert(Array{clblasDoubleComplex,2}, [hcat(11, 12, 13, 14, 15);hcat(21, 22, 23, 24, 25);hcat(31, 32, 33, 34, 35);hcat(41, 42, 43, 44, 45);hcat(81, 22, 33, 14, 75)]);
	X = convert(Array{clblasDoubleComplex,1}, [11,21,31,41,51,91]);
	Y = convert(Array{clblasDoubleComplex,1}, [45,23,39,45,50,10]);
	...
	incX = cl_int(1);
	incY = cl_int(1);
	alpha = clblasDoubleComplex(10);
	#Now initialize OpenCLBLAS and buffers
	statusCheck(clblasSetup())
	statusCheck(clFlush(queue[1]))
	err = Array(cl_int, 1)
	bufA = clCreateBuffer(ctx, CL_MEM_READ_WRITE, M * N * sizeof(clblasDoubleComplex), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
	bufX = clCreateBuffer(ctx, CL_MEM_READ_ONLY, M * sizeof(clblasDoubleComplex), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
	...
	event[1] = C_NULL
	statusCheck(clEnqueueWriteBuffer(queue[1], bufA, CL_TRUE, Csize_t(0), M * N * sizeof(clblasDoubleComplex), A.', cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	event[1] = C_NULL
	statusCheck(clEnqueueWriteBuffer(queue[1], bufX, CL_TRUE, Csize_t(0), M * sizeof(clblasDoubleComplex), X, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	event[1] = C_NULL
...
=====#
	...
	statusCheck(clblasZgerc(order, M, N, alpha, bufX, 0, incX, bufY, 0, incY, bufA, 0, lda, 1, queue, 0, C_NULL, event))

	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory

	A2=Array(clblasDoubleComplex, M * N)
	event[1] = C_NULL
	statusCheck(clEnqueueReadBuffer(queue[1], bufA, CL_TRUE, Csize_t(0), M * N * sizeof(clblasDoubleComplex), A2, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
