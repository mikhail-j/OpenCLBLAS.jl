#=**
* Ported from OpenCLBLAS zTBMV example (example_stbmv.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasZtbmv(o,ul,t,diag,N,K,A,offA,lda,X,offX,incX,sb,ncq,cq,ne,wle,e)
	return ccall((:clblasZtbmv, libclblas), cl_int, (clblasOrder,
		clblasUplo,
		clblasTranspose,
		clblasDiag,
		Csize_t,#N
...
function main()
	...
	statusCheck(err[1])
	################################	create arrays
	A =	convert(Array{clblasDoubleComplex,2}, [hcat(11, 12, 13);hcat(22, 23, 24);hcat(33, 34, 35);hcat(44, 45, 00);hcat(55, 00, 00)])
	...
	statusCheck(err[1])
	err = Array(cl_int, 1)
	bufX = clCreateBuffer(ctx, CL_MEM_WRITE_ONLY, N * sizeof(clblasDoubleComplex), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
	sbuf = clCreateBuffer(ctx, CL_MEM_READ_ONLY, N * sizeof(clblasDoubleComplex), C_NULL, err)
	statusCheck(err[1])
	statusCheck(clFlush(queue[1]))
	...
	event[1] = C_NULL
	statusCheck(clEnqueueWriteBuffer(queue[1], bufA, CL_TRUE, Csize_t(0), N * lda * sizeof(clblasDoubleComplex), transposeNC(A), cl_uint(0), C_NULL, event))
...
=====#
	...
	statusCheck(clblasZtbmv(order, uplo, trans, diag, N, K, bufA, 0, lda, bufX, 0, incX, sbuf, 1, queue, 0, C_NULL, event))
