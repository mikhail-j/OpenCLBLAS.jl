#=**
* Ported from OpenCLBLAS dSWAP example (example_sswap.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasDswap(N,X,offx,incX,Y,offy,incy,ncq,cq,ne,wle,e)
	return ccall((:clblasDswap, libclblas), cl_int, (Csize_t,
		cl_mem,#X
		Csize_t,
		cl_int,
		cl_mem,#Y
...
function main()
	...
	X = [cl_double(11),cl_double(21),cl_double(31),cl_double(41),cl_double(51),cl_double(61),cl_double(71)]
	Y = [cl_double(45),cl_double(23),cl_double(39),cl_double(45),cl_double(50),cl_double(55),cl_double(65)]
...
=====#
	...
	statusCheck(clblasDswap(N, bufX, 0, incX, bufY, 0, incY, 1, queue, 0, C_NULL, event))
	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	...
	println(string("X: ",typeof(X),X))
	println(string("Y: ",typeof(Y),Y))
	return
end
