#=**
* Ported from OpenCLBLAS sGER example (example_sger.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasSger(o,M,N,alpha,X,offX,incX,Y,offY,A,offA,lda,incY,ncq,cq,ne,wle,e)
	return ccall((:clblasSger, libclblas), cl_int, (clblasOrder,
		Csize_t,#M
		Csize_t,#N
		cl_float,#alpha
		cl_mem,#X
...
function main()
	...
	statusCheck(err[1])
	################################	create arrays
	A =	convert(Array{cl_float,2}, [hcat(11, 12, 13, 14, 15);hcat(21, 22, 23, 24, 25);hcat(31, 32, 33, 34, 35);hcat(41, 42, 43, 44, 45);hcat(81, 22, 33, 14, 75)]);
	X = convert(Array{cl_float,1}, [11,21,31,41,51,91]);
	Y = convert(Array{cl_float,1}, [45,23,39,45,50,10]);
	...
	err = Array(cl_int, 1)
	bufA = clCreateBuffer(ctx, CL_MEM_READ_WRITE, M * N * sizeof(cl_float), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
	bufX = clCreateBuffer(ctx, CL_MEM_READ_ONLY, M * sizeof(cl_float), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
	bufY = clCreateBuffer(ctx, CL_MEM_READ_ONLY, N * sizeof(cl_float), C_NULL, err)
	statusCheck(err[1])
	statusCheck(clFlush(queue[1]))
	...
	event[1] = C_NULL
	statusCheck(clEnqueueWriteBuffer(queue[1], bufX, CL_TRUE, Csize_t(0), M * sizeof(cl_float), X, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	event[1] = C_NULL
...
=====#
	...
	statusCheck(clblasSger(order, M, N, alpha, bufX, 0, incX, bufY, 0, incY, bufA, 0, lda, 1, queue, 0, C_NULL, event))

	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory

	A2=Array(cl_float, M * N)
	event[1] = C_NULL
	statusCheck(clEnqueueReadBuffer(queue[1], bufA, CL_TRUE, Csize_t(0), M * N * sizeof(cl_float), A2, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
