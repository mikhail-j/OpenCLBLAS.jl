#=**
* Ported from OpenCLBLAS izAMAX example (example_isamax.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasiZamax(N,Iamax,offIamax,X,offX,incX,sb,ncq,cq,ne,wle,e)
	return ccall((:clblasiZamax, libclblas), clblasStatus, (Csize_t,
...
function main()
	...
	statusCheck(err[1])
	################################	create arrays
	X = convert(Array{clblasDoubleComplex, 1}, [1,2,-11,17,5,6,800,10]);
	incX = cl_int(1);
	N = Csize_t(length(X));
...
=====#
	...
	statusCheck(clblasiZamax(N, bufAMAX, 0, bufX, 0, incX, sbuf, 1, queue, 0, C_NULL, event))
