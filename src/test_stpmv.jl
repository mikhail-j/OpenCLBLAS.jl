#=**
* Ported from OpenCLBLAS sTPMV example (example_stpmv.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasStpmv(o,ul,tA,diag,N,A,offA,X,offX,incX,sb,ncq,cq,ne,wle,e)
	return ccall((:clblasStpmv, libclblas), cl_int, (clblasOrder,
		clblasUplo,
		clblasTranspose,
		clblasDiag,
		Csize_t,#N
...
function main()
	...
	statusCheck(err[1])
	################################	create arrays
	A =	convert(Array{cl_float,1}, [11, 12, 13, 14, 15, 22, 23, 24, 25, 33, 34, 35, 44, 45, 55])
...
=====#
	...
	statusCheck(clblasStpmv(order, uploA, transA, diagA,  N, bufA, 0, bufX, 0, incX, sbuf, 1, queue, 0, C_NULL, event))
