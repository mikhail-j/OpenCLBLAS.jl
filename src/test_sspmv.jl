#=**
* Ported from OpenCLBLAS sSPMV example (example_sspmv.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasSspmv(o,ul,N,alpha,A,offA,X,offX,incX,beta,Y,offY,incY,ncq,cq,ne,wle,e)
	return ccall((:clblasSspmv, libclblas), cl_int, (clblasOrder,
		clblasUplo,
		Csize_t,#N
		cl_float,#alpha
		cl_mem,#A
...
function main()
	...
	statusCheck(err[1])
	################################	create arrays
	A =	convert(Array{cl_float,1}, vec(hcat(hcat(11, 12, 13, 14, 15),hcat(22, 23, 24, 25),hcat(33, 34, 35),hcat(44, 45),hcat(55))));
...
=====#
	...
	statusCheck(clblasSspmv(order, uplo, N, alpha, bufA, 0, bufX, 0, incX, beta, bufY, 0, incY, 1, queue, 0, C_NULL, event))
