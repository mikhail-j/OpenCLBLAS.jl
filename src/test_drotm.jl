#=**
* Ported from OpenCLBLAS dROTM example (example_srotm.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasDrotm(N,X,offX,incX,Y,offY,incY,param,offparam,ncq,cq,ne,wle,e)
	return ccall((:clblasDrotm, libclblas), clblasStatus, (Csize_t,
		cl_mem,#X
		Csize_t,
		cl_int,
		cl_mem,#Y
...
function main()
	...
	statusCheck(err[1])
	################################	create arrays
	X = convert(Array{cl_double,1}, [11,21,31,41,51,61,71]);
	Y = convert(Array{cl_double,1}, [15,11,1,2,1,8,1]);
	incX = cl_int(1);
	incY = cl_int(1);
	N = Csize_t(length(X));
...
=====#
	...
	statusCheck(clblasDrotm(N, bufX, 0, incX, bufY, 0, incY, bufP, 0, 1, queue, 0, C_NULL, event))
