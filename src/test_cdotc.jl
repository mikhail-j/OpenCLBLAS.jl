#=**
* Ported from OpenCLBLAS cDOTc example (example_sdot.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasCdotc(N,DP,offDP,X,offX,incX,Y,offY,incY,sb,ncq,cq,ne,wle,e)
	return ccall((:clblasCdotc, libclblas), cl_int, (Csize_t,#N
		cl_mem,#dot product
		Csize_t,
		cl_mem,#X
		Csize_t,
...
=====#
	event[1] = C_NULL
	statusCheck(clblasCdotc(N, bufDot, Csize_t(0), bufX, Csize_t(0), incX, bufY, Csize_t(0), incY, sbuf,1, queue, 0, C_NULL, event))
	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
