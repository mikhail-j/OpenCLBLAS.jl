#=**
* Ported from OpenCLBLAS cSWAP example (example_sswap.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasCswap(N,X,offx,incX,Y,offy,incy,ncq,cq,ne,wle,e)
	return ccall((:clblasCswap, libclblas), cl_int, (Csize_t,
		cl_mem,#X
		Csize_t,
		cl_int,
		cl_mem,#Y
...
function main()
	...
	X = [clblasFloatComplex(11),clblasFloatComplex(21),clblasFloatComplex(31),clblasFloatComplex(41),clblasFloatComplex(51),clblasFloatComplex(61),clblasFloatComplex(71)]
	Y = [clblasFloatComplex(45),clblasFloatComplex(23),clblasFloatComplex(39),clblasFloatComplex(45),clblasFloatComplex(50),clblasFloatComplex(55),clblasFloatComplex(65)]
...
=====#
	...
	statusCheck(clblasCswap(N, bufX, 0, incX, bufY, 0, incY, 1, queue, 0, C_NULL, event))
	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	...
	event[1] = C_NULL
	statusCheck(clEnqueueReadBuffer(queue[1], bufX, CL_TRUE, Csize_t(0), N * sizeof(clblasFloatComplex), X, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	event[1] = C_NULL
	statusCheck(clEnqueueReadBuffer(queue[1], bufY, CL_TRUE, Csize_t(0), N * sizeof(clblasFloatComplex), Y, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
