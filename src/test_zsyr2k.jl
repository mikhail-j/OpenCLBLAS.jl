#=**
* Ported from OpenCLBLAS zSYR2K example (example_ssyr2k.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasZsyr2k(o,ul,t,N,K,alpha,A,offA,lda,B,offB,ldb,beta,C,offC,ldc,ncq,cq,ne,wle,e)
	return ccall((:clblasZsyr2k, libclblas), cl_int, (clblasOrder,
		clblasUplo,
		clblasTranspose,
		Csize_t,#N
		Csize_t,#K
...
function main()
	...
	statusCheck(err[1])
	################################	create arrays
	A =	convert(Array{clblasDoubleComplex,2}, [hcat(11, 12, 13, 14);hcat(21, 22, 23, 24);hcat(31, 32, 33, 34);hcat(41, 42, 43, 44);hcat(51, 52, 53, 54)]);
	B =	convert(Array{clblasDoubleComplex,2}, [hcat(11, 12, 13, 14);hcat(21, 22, 23, 24);hcat(31, 32, 33, 34);hcat(41, 42, 43, 44);hcat(51, 52, 53, 54)]);
	C = convert(Array{clblasDoubleComplex,2}, [hcat(11, 12, 13, 14, 15);hcat(12, 22, 23, 24, 25);hcat(13, 23, 33, 34, 35);hcat(14, 24, 34, 44, 45);hcat(15, 25, 35, 45, 55)]);
	...
	alpha[1] = clblasDoubleComplex(10);
	beta = Array(clblasDoubleComplex, 1);
	beta[1] = clblasDoubleComplex(20);
	...
	err = Array(cl_int, 1)
	bufA = clCreateBuffer(ctx, CL_MEM_READ_ONLY, K * N * sizeof(clblasDoubleComplex), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
	...
	event[1] = C_NULL
	statusCheck(clEnqueueWriteBuffer(queue[1], bufA, CL_TRUE, Csize_t(0), N * K * sizeof(clblasDoubleComplex), transposeNC(A), cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	event[1] = C_NULL
	statusCheck(clEnqueueWriteBuffer(queue[1], bufB, CL_TRUE, Csize_t(0), N * K * sizeof(clblasDoubleComplex), transposeNC(B), cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	event[1] = C_NULL
	statusCheck(clEnqueueWriteBuffer(queue[1], bufC, CL_TRUE, Csize_t(0), N * N * sizeof(clblasDoubleComplex), transposeNC(C), cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
...
=====#
	...
	statusCheck(clblasZsyr2k(order, uplo, transAB, N, K, alpha, bufA, 0, lda, bufB, 0, ldb, beta, bufC, 0, ldc, 1, queue, 0, C_NULL, event))
