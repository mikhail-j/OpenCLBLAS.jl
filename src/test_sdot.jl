#=**
* Ported from OpenCLBLAS sDOT example (example_sdot.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasSdot(N,DP,offDP,X,offX,incX,Y,offY,incY,sb,ncq,cq,ne,wle,e)
	return ccall((:clblasSdot, libclblas), cl_int, (Csize_t,#N
		cl_mem,#dot product
		Csize_t,
		cl_mem,#X
		Csize_t,
...
function main()
	...
	statusCheck(err[1])
	################################	create arrays
	X = convert(Array{cl_float, 1}, [1,2,-11,17,5,6,81]);
	Y = convert(Array{cl_float, 1}, [1,5,6,4,9,10,4]);
	N = Csize_t(length(X));
	incX = cl_int(1);
	incY = cl_int(1);
	local dotP = zeros(cl_float,1);
	#Now initialize OpenCLBLAS and buffers
	statusCheck(clblasSetup())
	statusCheck(clFlush(queue[1]))
	err = Array(cl_int, 1)
	local bufX = clCreateBuffer(ctx, CL_MEM_READ_ONLY, N * sizeof(cl_float), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
	local bufY = clCreateBuffer(ctx, CL_MEM_READ_ONLY, N * sizeof(cl_float), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
	local sbuf = clCreateBuffer(ctx, CL_MEM_READ_WRITE, N * sizeof(cl_float), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
	local bufDot = clCreateBuffer(ctx, CL_MEM_WRITE_ONLY, sizeof(cl_float), C_NULL, err)
	statusCheck(err[1])
	statusCheck(clFlush(queue[1]))
...
=====#
	event[1] = C_NULL
	statusCheck(clblasSdot(N, bufDot, Csize_t(0), bufX, Csize_t(0), incX, bufY, Csize_t(0), incY, sbuf,1, queue, 0, C_NULL, event))
	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	...
	event[1] = C_NULL
	statusCheck(clEnqueueReadBuffer(queue[1], bufDot, CL_TRUE, Csize_t(0), sizeof(cl_float), dotP, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
