#=**
* Ported from OpenCLBLAS scASUM example (example_sasum.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasScasum(N,as,offA,X,offX,incX,sb,ncq,cq,ne,wle,e)
	return ccall((:clblasScasum, libclblas), cl_int, (Csize_t,#N
		cl_mem,#absolute sum
		Csize_t,
		cl_mem,#X
		Csize_t,
...
function main()
	...
	statusCheck(err[1])
	################################	create arrays
	X = vec([clblasFloatComplex(1),clblasFloatComplex(2),clblasFloatComplex(-11),clblasFloatComplex(17),clblasFloatComplex(5),clblasFloatComplex(6),clblasFloatComplex(81)]');
	N = Csize_t(length(X));
	incX = cl_int(1);
	local asum = zeros(cl_float,1)
	...
	statusCheck(err[1])
	err = Array(cl_int, 1)
	local bufASUM = clCreateBuffer(ctx, CL_MEM_WRITE_ONLY, sizeof(clblasFloatComplex), C_NULL, err)
	statusCheck(err[1])
	statusCheck(clFlush(queue[1]))
...
=====#
	event[1] = C_NULL
	statusCheck(clblasScasum(N, bufASUM, Csize_t(0), bufX, Csize_t(0), incX, sbuf, 1, queue, 0, C_NULL, event))
	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	...
	event[1] = C_NULL
	statusCheck(clEnqueueReadBuffer(queue[1], bufASUM, CL_TRUE, Csize_t(0), sizeof(cl_float), asum, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory

#=
	scratch = zeros(clblasFloatComplex, N)
	event[1] = C_NULL
	statusCheck(clEnqueueReadBuffer(queue[1], sbuf, CL_TRUE, Csize_t(0), N * sizeof(clblasFloatComplex), scratch, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	println(scratch)
=#
