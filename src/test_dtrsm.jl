#=**
* Ported from OpenCLBLAS dTRSM example (example_strsm.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasDtrsm(o,side,ul,tA,diag,M,N,alpha,A,offA,lda,B,offB,ldb,ncq,cq,ne,wle,e)
	return ccall((:clblasDtrsm, libclblas), cl_int, (clblasOrder,
		clblasSide,
		clblasUplo,
		clblasTranspose,
		clblasDiag,
...
function main()
	...
	statusCheck(err[1])
	################################	create arrays
	A =	convert(Array{cl_double,2}, [hcat(11, 12, 13, 14);hcat(0, 22, 23, 24);hcat(0,  0, 33, 34);hcat(0,  0,  0, 44)])
	B = convert(Array{cl_double,2}, [hcat(11, 12, 13, 14, 15);hcat(21, 22, 23, 24, 25);hcat(31, 32, 33, 34, 35);hcat(41, 42, 43, 44, 45)])
	...
	side = clblasLeft
	#alpha = Array(cl_double, 1)
	#alpha[1] = convert(cl_double, 10)
	...
	statusCheck(err[1])
	err = Array(cl_int, 1)
	bufB = clCreateBuffer(ctx, CL_MEM_READ_WRITE, M * N * sizeof(cl_double), C_NULL, err)
	statusCheck(err[1])
	statusCheck(clFlush(queue[1]))
...
=====#
	...
	statusCheck(clblasDtrsm(order, side, uploA, transA, diagA, M, N,
	...
	B2=Array(cl_double,length(B))
	event[1] = C_NULL
	statusCheck(clEnqueueReadBuffer(queue[1], bufB, CL_TRUE, Csize_t(0), M * N * sizeof(cl_double), B2, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
