#=**
* Ported from OpenCLBLAS cROTG example (example_srotg.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasCrotg(A,offA,B,offB,C,offC,S,offS,ncq,cq,ne,wle,e)
	return ccall((:clblasCrotg, libclblas), clblasStatus, (cl_mem,#A
		Csize_t,
		cl_mem,#B
		Csize_t,
		cl_mem,#C
...
function main()
	...
	statusCheck(err[1])
	################################	create arrays
	A = collect(clblasFloatComplex(11));
	B = collect(clblasFloatComplex(21));
	C = collect(clblasFloatComplex(0.2));
	S = collect(clblasFloatComplex(0.5));

	#Now initialize OpenCLBLAS and buffers
	statusCheck(clblasSetup())
	statusCheck(clFlush(queue[1]))
	err = Array(cl_int, 1)
	bufA = clCreateBuffer(ctx, CL_MEM_READ_WRITE, sizeof(clblasFloatComplex), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
	bufB = clCreateBuffer(ctx, CL_MEM_READ_WRITE, sizeof(clblasFloatComplex), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
	bufC = clCreateBuffer(ctx, CL_MEM_READ_WRITE, sizeof(clblasFloatComplex), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
	bufS = clCreateBuffer(ctx, CL_MEM_READ_WRITE, sizeof(clblasFloatComplex), C_NULL, err)
	statusCheck(err[1])
	statusCheck(clFlush(queue[1]))
	...
	event[1] = C_NULL
	statusCheck(clEnqueueWriteBuffer(queue[1], bufA, CL_TRUE, Csize_t(0), sizeof(clblasFloatComplex), A, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	event[1] = C_NULL
	statusCheck(clEnqueueWriteBuffer(queue[1], bufB, CL_TRUE, Csize_t(0), sizeof(clblasFloatComplex), B, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	event[1] = C_NULL
	statusCheck(clEnqueueWriteBuffer(queue[1], bufC, CL_TRUE, Csize_t(0), sizeof(clblasFloatComplex), C, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	event[1] = C_NULL
	statusCheck(clEnqueueWriteBuffer(queue[1], bufS, CL_TRUE, Csize_t(0), sizeof(clblasFloatComplex), S, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
...
=====#
	...
	statusCheck(clblasCrotg(bufA, 0, bufB, 0, bufC, 0, bufS, 0, 1, queue, 0, C_NULL, event))
	...
	event[1] = C_NULL
	statusCheck(clEnqueueReadBuffer(queue[1], bufA, CL_TRUE, Csize_t(0), sizeof(clblasFloatComplex), A, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	event[1] = C_NULL
	statusCheck(clEnqueueReadBuffer(queue[1], bufB, CL_TRUE, Csize_t(0), sizeof(clblasFloatComplex), B, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	event[1] = C_NULL
	statusCheck(clEnqueueReadBuffer(queue[1], bufC, CL_TRUE, Csize_t(0), sizeof(clblasFloatComplex), C, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	event[1] = C_NULL
	statusCheck(clEnqueueReadBuffer(queue[1], bufS, CL_TRUE, Csize_t(0), sizeof(clblasFloatComplex), S, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
