#=**
* Ported from OpenCLBLAS dSYMM example (example_ssymm.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasDsymm(o,side,ul,M,N,alpha,A,offA,lda,B,offB,ldb,beta,C,offC,ldc,ncq,cq,ne,wle,e)
	return ccall((:clblasDsymm, libclblas), cl_int, (clblasOrder,
		clblasSide,
		clblasUplo,
		Csize_t,#M
		Csize_t,#N
...
function main()
	...
	statusCheck(err[1])
	################################	create arrays
	A =	convert(Array{cl_double,2}, [hcat(11, -1, -1, -1);hcat(21, 22, -1, -1);hcat(31, 32, 33, -1);hcat(41, 42, 43, 44)]);
	B = convert(Array{cl_double,2}, [hcat(11, 12, 13);hcat(21, 22, 23);hcat(31, 32, 33);hcat(41, 42, 43)]);
	C = convert(Array{cl_double,2}, [hcat(11, 12, 13);hcat(21, 22, 23);hcat(31, 32, 33);hcat(41, 42, 43)]);
	...
	err = Array(cl_int, 1)
	bufA = clCreateBuffer(ctx, CL_MEM_READ_ONLY, M * M * sizeof(cl_double), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
	bufB = clCreateBuffer(ctx, CL_MEM_READ_ONLY, M * N * sizeof(cl_double), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
	...
	event[1] = C_NULL
	statusCheck(clEnqueueWriteBuffer(queue[1], bufA, CL_TRUE, Csize_t(0), M * M * sizeof(cl_double), transposeNC(A), cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	event[1] = C_NULL
	statusCheck(clEnqueueWriteBuffer(queue[1], bufB, CL_TRUE, Csize_t(0), M * N * sizeof(cl_double), transposeNC(B), cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	event[1] = C_NULL
	statusCheck(clEnqueueWriteBuffer(queue[1], bufC, CL_TRUE, Csize_t(0), M * N * sizeof(cl_double), transposeNC(C), cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
...
=====#
	...
	statusCheck(clblasDsymm(order, side, uplo, M, N, alpha, bufA, 0, lda, bufB, 0, ldb, beta, bufC, 0, ldc, 1, queue, 0, C_NULL, event))

	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory

	C2=Array(cl_double, M * N)
	event[1] = C_NULL
	statusCheck(clEnqueueReadBuffer(queue[1], bufC, CL_TRUE, Csize_t(0), M * N * sizeof(cl_double), C2, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
