#=**
* Ported from OpenCLBLAS dSCAL example (example_sscal.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasDscal(N,alpha,X,offx,incX,ncq,cq,ne,wle,e)
	return ccall((:clblasDscal, libclblas), cl_int, (Csize_t,
		cl_double,
		cl_mem,#X
		Csize_t,
		cl_int,
...
function main()
	...
	X = [cl_double(11),cl_double(21),cl_double(31),cl_double(41),cl_double(51)]
...
=====#
	...
	statusCheck(clblasDscal(N, alpha, bufX, 0, incX, 1, queue, 0, C_NULL, event))
	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory

	X1=vec(Array(cl_double,N))
	event[1] = C_NULL
