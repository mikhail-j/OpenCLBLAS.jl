#=**
* Ported from OpenCLBLAS dNRM2 example (example_snrm2.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasDnrm2(N,nrm2,offnrm2,X,offX,incX,sb,ncq,cq,ne,wle,e)
	return ccall((:clblasDnrm2, libclblas), clblasStatus, (Csize_t,#N
		cl_mem,#nrm2
		Csize_t,
		cl_mem,#X
		Csize_t,
		...
		Ptr{cl_event},
		Ptr{cl_event}),
		N,nrm2,offnrm2,X,offX,incX,sb,ncq,cq,ne,wle,e)
end
...
function main()
	...
	statusCheck(err[1])
	err = Array(cl_int, 1)
	bufNRM2 = clCreateBuffer(ctx, CL_MEM_WRITE_ONLY, sizeof(cl_double), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
	sbuf = clCreateBuffer(ctx, CL_MEM_READ_WRITE, 2 * N * sizeof(cl_double), C_NULL, err)
	statusCheck(err[1])
	statusCheck(clFlush(queue[1]))
...
=====#
	...
	statusCheck(clblasDnrm2(N, bufNRM2, 0, bufX, 0, incX, sbuf, 1, queue, 0, C_NULL, event));

	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory

	local NRM2 = zeros(cl_double,1);
	event[1] = C_NULL
	statusCheck(clEnqueueReadBuffer(queue[1], bufNRM2, CL_TRUE, Csize_t(0), sizeof(cl_double), NRM2, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory

	statusCheck(clFlush(queue[1]))
	statusCheck(clReleaseMemObject(bufNRM2))
	statusCheck(clFlush(queue[1]))
