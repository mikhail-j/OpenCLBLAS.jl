#=**
* Ported from OpenCLBLAS dGEMV example (example_sgemv.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasDgemv(o,tA,M,N,alpha,A,offA,lda,X,offX,incX,beta,Y,offY,incY,ncq,cq,ne,wle,e)
	return ccall((:clblasDgemv, libclblas), cl_int, (clblasOrder,
		clblasTranspose,
		Csize_t,#M
		Csize_t,#N
		cl_double,#alpha
...
function main()
	...
	statusCheck(err[1])
	################################	create arrays
	A =	convert(Array{cl_double,2}, [hcat(11, 12, 13, 14, 15);hcat(21, 22, 23, 24, 25);hcat(31, 32, 33, 34, 35);hcat(41, 42, 43, 44, 45)]);
	X = convert(Array{cl_double,1}, [11,21,31,41,51]);
	Y = convert(Array{cl_double,1}, [11,21,31,41]);
	...
	err = Array(cl_int, 1)
	bufA = clCreateBuffer(ctx, CL_MEM_READ_ONLY, M * N * sizeof(cl_double), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
...
=====#
	...
	statusCheck(clblasDgemv(order, transA, M, N, alpha, bufA, 0, lda, bufX, 0, incX, beta, bufY, 0, incY, 1, queue, 0, C_NULL, event))

	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory

	Y2=Array(cl_double, M)
	event[1] = C_NULL
	statusCheck(clEnqueueReadBuffer(queue[1], bufY, CL_TRUE, Csize_t(0), M * sizeof(cl_double), Y2, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
