#=**
* Ported from OpenCLBLAS cCOPY example (example_scopy.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasCcopy(N,X,offX,incX,Y,offY,incY,ncq,cq,ne,wle,e)
	return ccall((:clblasCcopy, libclblas), cl_int, (Csize_t,#N
		cl_mem,#X
		Csize_t,
		Cint,
		cl_mem,#Y
...
function main()
	...
	statusCheck(err[1])
	################################	create arrays
	X = convert(Array{clblasFloatComplex, 1}, [11,21,31,41,51,61,71]);
	Y = convert(Array{clblasFloatComplex, 1}, [0,2,0,0,0,5,0]);
	N = Csize_t(length(X));
	incX = cl_int(1);
	incY = cl_int(1);
	...
	statusCheck(err[1])
	err = Array(cl_int, 1)
	local bufY = clCreateBuffer(ctx, CL_MEM_READ_WRITE, N * sizeof(clblasFloatComplex), C_NULL, err)
	statusCheck(err[1])
	statusCheck(clFlush(queue[1]))
...
=====#
	event[1] = C_NULL
	statusCheck(clblasCcopy(N, bufX, Csize_t(0), incX, bufY, Csize_t(0), incY, 1, queue, cl_uint(0), C_NULL, event))
	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
