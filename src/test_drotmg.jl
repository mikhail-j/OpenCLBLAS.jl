#=**
* Ported from OpenCLBLAS dROTMG example (example_srotmg.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasDrotmg(D1,offD1,D2,offD2,X1,offX1,Y1,offY1,param,offparam,ncq,cq,ne,wle,e)
	return ccall((:clblasDrotmg, libclblas), clblasStatus, (cl_mem,
		Csize_t,
		cl_mem,
		Csize_t,
		cl_mem,
...
function main()
	...
	statusCheck(err[1])
	################################	create arrays
	D1 = collect(cl_double(10));
	D2 = collect(cl_double(21));
	X1 = collect(cl_double(1));
	Y1 = collect(cl_double(-1));
	PARAM = convert(Array{cl_double, 1}, [-1,10,12,20,2]);

	#Now initialize OpenCLBLAS and buffers
	statusCheck(clblasSetup())
	statusCheck(clFlush(queue[1]))
	err = Array(cl_int, 1)
	bufD1 = clCreateBuffer(ctx, CL_MEM_READ_WRITE, sizeof(cl_double), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
	bufD2 = clCreateBuffer(ctx, CL_MEM_READ_WRITE, sizeof(cl_double), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
	bufX1 = clCreateBuffer(ctx, CL_MEM_READ_WRITE, sizeof(cl_double), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
	bufY1 = clCreateBuffer(ctx, CL_MEM_READ_WRITE, sizeof(cl_double), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
	bufP = clCreateBuffer(ctx, CL_MEM_READ_WRITE, length(PARAM) * sizeof(cl_double), C_NULL, err)
	statusCheck(err[1])
	statusCheck(clFlush(queue[1]))
	...
	event[1] = C_NULL
	statusCheck(clEnqueueWriteBuffer(queue[1], bufD1, CL_TRUE, Csize_t(0), sizeof(cl_double), D1, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	event[1] = C_NULL
	statusCheck(clEnqueueWriteBuffer(queue[1], bufD2, CL_TRUE, Csize_t(0), sizeof(cl_double), D2, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	event[1] = C_NULL
	statusCheck(clEnqueueWriteBuffer(queue[1], bufX1, CL_TRUE, Csize_t(0), sizeof(cl_double), X1, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	event[1] = C_NULL
	statusCheck(clEnqueueWriteBuffer(queue[1], bufY1, CL_TRUE, Csize_t(0), sizeof(cl_double), Y1, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	event[1] = C_NULL
	statusCheck(clEnqueueWriteBuffer(queue[1], bufP, CL_TRUE, Csize_t(0), length(PARAM) * sizeof(cl_double), PARAM, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
...
=====#
	...
	statusCheck(clblasDrotmg(bufD1, 0, bufD2, 0, bufX1, 0, bufY1, 0, bufP, 0, 1, queue, 0, C_NULL, event))
	...
	event[1] = C_NULL
	statusCheck(clEnqueueReadBuffer(queue[1], bufD1, CL_TRUE, Csize_t(0), sizeof(cl_double), D1, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	event[1] = C_NULL
	statusCheck(clEnqueueReadBuffer(queue[1], bufD2, CL_TRUE, Csize_t(0), sizeof(cl_double), D2, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	event[1] = C_NULL
	statusCheck(clEnqueueReadBuffer(queue[1], bufX1, CL_TRUE, Csize_t(0), sizeof(cl_double), X1, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	event[1] = C_NULL
	statusCheck(clEnqueueReadBuffer(queue[1], bufY1, CL_TRUE, Csize_t(0), sizeof(cl_double), Y1, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	event[1] = C_NULL
	statusCheck(clEnqueueReadBuffer(queue[1], bufP, CL_TRUE, Csize_t(0), length(PARAM) * sizeof(cl_double), PARAM, cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
