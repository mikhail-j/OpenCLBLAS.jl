#=**
* Ported from OpenCLBLAS zSYMM example (example_ssymm.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasZsymm(o,side,ul,M,N,alpha,A,offA,lda,B,offB,ldb,beta,C,offC,ldc,ncq,cq,ne,wle,e)
	return ccall((:clblasZsymm, libclblas), cl_int, (clblasOrder,
		clblasSide,
		clblasUplo,
		Csize_t,#M
		Csize_t,#N
...
function main()
	...
	statusCheck(err[1])
	################################	create arrays
	A =	convert(Array{clblasDoubleComplex,2}, [hcat(11, -1, -1, -1);hcat(21, 22, -1, -1);hcat(31, 32, 33, -1);hcat(41, 42, 43, 44)]);
	B = convert(Array{clblasDoubleComplex,2}, [hcat(11, 12, 13);hcat(21, 22, 23);hcat(31, 32, 33);hcat(41, 42, 43)]);
	C = convert(Array{clblasDoubleComplex,2}, [hcat(11, 12, 13);hcat(21, 22, 23);hcat(31, 32, 33);hcat(41, 42, 43)]);
	...
	statusCheck(clReleaseEvent(event[1]))		#free the memory
	event[1] = C_NULL
	statusCheck(clEnqueueWriteBuffer(queue[1], bufC, CL_TRUE, Csize_t(0), M * N * sizeof(clblasDoubleComplex), transposeNC(C), cl_uint(0), C_NULL, event))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
...
=====#
	...
	statusCheck(clblasZsymm(order, side, uplo, M, N, alpha, bufA, 0, lda, bufB, 0, ldb, beta, bufC, 0, ldc, 1, queue, 0, C_NULL, event))

	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory

	C2=Array(clblasDoubleComplex, M * N)
	event[1] = C_NULL
