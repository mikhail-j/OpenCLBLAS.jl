#=**
* Ported from OpenCLBLAS sASUM example (example_sasum.c)
* Qijia (Michael) Jin
* @version 0.0.1
*
...
function clblasSasum(N,as,offA,X,offX,incX,sb,ncq,cq,ne,wle,e)
	return ccall((:clblasSasum, libclblas), cl_int, (Csize_t,#N
		cl_mem,#absolute sum
		Csize_t,
		cl_mem,#X
		Csize_t,
...
function main()
	...
	statusCheck(err[1])
	err = Array(cl_int, 1)
	sbuf = clCreateBuffer(ctx, CL_MEM_READ_WRITE, N * sizeof(cl_float), C_NULL, err)
	statusCheck(err[1])
	err = Array(cl_int, 1)
	bufASUM = clCreateBuffer(ctx, CL_MEM_WRITE_ONLY, sizeof(cl_float), C_NULL, err)
	statusCheck(err[1])
	statusCheck(clFlush(queue[1]))
...
=====#
	event[1] = C_NULL
	statusCheck(clblasSasum(N, bufASUM, 0, bufX, 0, incX, sbuf, 1, queue, 0, C_NULL, event))
	statusCheck(clFlush(queue[1]))
	statusCheck(clWaitForEvents(1,event))
	statusCheck(clReleaseEvent(event[1]))		#free the memory
